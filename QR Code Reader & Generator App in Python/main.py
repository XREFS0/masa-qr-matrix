"""
MASA 05_QR Code Reader & Generator App in Python with Source Code
Developer: MASA
"""
...
def close_window():
    if askyesno(
        title="Close QR Code Reader & Generator App",
...
def generate_qrcode():
    ...
    if qrcode_name == "":
        ...
        if askyesno(
            title="Confirmation", message=f"Do you want to create a QRCode with the provided information?"
        ):
            try:
...
root = Tk()
root.title("MASA QR Matrix Studio")
